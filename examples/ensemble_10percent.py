...
from mct.utils import subset_npercent, IndexedDataset, ConcatDataset
...
@torch.no_grad()
def accuracy_callback(model, loader, name='train', dtype=torch.float32):
        metric = Accuracy()
        model.eval()
        ...
        for batch in loader:
            ...
            out = model(x.to(torch.cuda.current_device()).type(dtype))
...
def training_process(args, rank, world_size):
    ...
    wandb.init(project=f'MCT ensemble {args.dataset}', entity='ai2es',
    ...
    models = [DDP(FCNN(shape, num_classes, [1024, 1024, 1024]).to(torch.cuda.current_device()), device_ids=[rank]) for _ in range(args.members)]
...
def create_parser():
    ...
    parser.add_argument('-m', '--members', type=int, default=3, 
    ...
    parser.add_argument('--path', type=str, default='/ourdisk/hpc/ai2es/jroth/AI2ES_DL_Torch/MCT/ens_hp2', help='path for hparam search directory')
    ...
    parser.add_argument('--train_size', type=float, default=[0.10],
