...
class MetaCoTrainingModel(torch.nn.Module):
    ...
    def train(self, epochs, warmup, train_views, unlbl_views, val_views, test_views, checkpoint_path, optimizer=Adam, batch_size=1024, lr=1e-4, lr_scheduler=ReduceLROnPlateau, patience=1, amp=True, use_wandb=False, log_interval=1, approx=True, supervised=True):
        ...
        for i in range(len(train_views)):
            ...
            train_views[i], sampler = make_loader(train_views[i], len(self.models), batch_size, shuffle=True)

            unlbl_views[i], sampler_unlbl = make_loader(unlbl_views[i], len(self.models), batch_size, shuffle=True)

            val_views[i], _ = make_loader(val_views[i], len(self.models), batch_size, shuffle=False)

            test_views[i], _ = make_loader(test_views[i], len(self.models), batch_size, shuffle=False)
        ...
        for epoch in range(epochs):
            ...
            if epoch < warmup:
                ...
                if epoch == warmup:
                    ...
                    model = model.to(int(os.environ['RANK']) % torch.cuda.device_count())
                ...
                for e, (U, X) in tqdm(zip(*[range(len(unlbl_views[0])), zip(zip(*[iter(RepeatLoader(v)) for v in unlbl_views]), zip(*[iter(RepeatLoader(v)) for v in train_views]))])):
                    ...
                    U = U.to(int(os.environ['RANK']) % torch.cuda.device_count())
                    ...
                    if self.s % log_interval == 0:
                        ...
                        torch.distributed.barrier()
                        d['c_acc_val'] = self.co_accuracy(self.val_views)
                        d['c_acc_test'] = self.co_accuracy(self.test_views)

                        scheduler.step(d[f'val_acc{i}'])

                        if states.get('c_acc_val') is None:
                            states['c_acc_val'] = d['c_acc_val']
                        elif d['c_acc_val'] > states.get('c_acc_val'):
                            states['c_acc_val'] = d['c_acc_val']
                            states[f'best_model{i}_state'] = model.state_dict()
    ...
    def mct(self, U, X, y, model, optimizer, scaler, loss=torch.nn.CrossEntropyLoss(), supervised=True, approx=False, amp=True):
        ...
        if self.s % mct_length < accum_steps:
            ...
            if (not self.self_loss_grads) or (self.loss_final == 0):
                ...
                print(self.self_loss_grads, self.loss_final)
                self.s = -1
                return
