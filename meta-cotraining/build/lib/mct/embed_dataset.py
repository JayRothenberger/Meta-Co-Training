...
class EsViT_distance(SSL_distance):
    ...
    EsVit distance
    """
    def __init__(self, chkpt_path='/scratch/jroth/esvit_swin_b.pth', model=vit_base_patch16):
        model = swin_b()
...
class CLIP_distance(SSL_distance):
    ...
    def __init__(self, model='ViT-L/14'):
        ...
        self.network = model.to(visible_device)
        self.model = self.network.encode_image
...
def write_embeds_to_file(fp, loader, dist):
    ...
    for i, (x, y) in enumerate(loader):
        with torch.no_grad():
            gc.collect()
            embeds.append(dist.embed(x).cpu())
            labels.append(y.cpu())
...
def main():
    parser = argparse.ArgumentParser(description='Embed Datasets to Files')
    parser.add_argument('--embedding', type=str, default='DINOv2', metavar='N', # NEW
                        help='embedding type')

    args = parser.parse_args()

    wandb.init(project='image embeddings', entity='change-this',
        name=f"{args.embedding}",
        config={
            'experiment': 'image embeddings',
            'args': vars(args)
        })

    dist = IMAGE_DISTANCES[args.embedding]()

    image_transform = IMAGE_TRANSFORMS[args.embedding]
    dataset1 = torchvision.datasets.ImageNet('../imagenet', split='train', transform=image_transform)
    dataset2 = torchvision.datasets.ImageNet('../imagenet', split='val', transform=image_transform)

    # dataset1 = torchvision.datasets.Food101('../Food101', split='train', transform=image_transform, download=True)
    # dataset2 = torchvision.datasets.Food101('../Food101', split='test', transform=image_transform, download=True)
    
    # this one requires only training embeddings and can be split later with process_iNat.py which takes much less time than this script
    # dataset1 = torchvision.datasets.INaturalist('../iNat2017', version='2017', transform=image_transform, download=False)

    # dataset1 = torchvision.datasets.INaturalist('../iNat', version= '2021_train', transform=image_transform, download=False)
    # dataset2 = torchvision.datasets.INaturalist('../iNat', version= '2021_valid', transform=image_transform, download=False)

    # dataset1 = torchvision.datasets.Flowers102('../Flowers', 'train', transform=image_transform, download=False)
    # dataset2 = torchvision.datasets.Flowers102('../Flowers', 'val', transform=image_transform, download=False)
    # dataset1 = torchvision.datasets.Flowers102('../Flowers', 'test', transform=image_transform, download=False)

    # dataset1 = torchvision.datasets.FGVCAircraft('../Aircraft', 'train', transform=image_transform, download=False)
    # dataset2 = torchvision.datasets.FGVCAircraft('../Aircraft', 'val', transform=image_transform, download=False)
    # dataset2 = torchvision.datasets.FGVCAircraft('../Aircraft', 'test', transform=image_transform, download=False)

    if args.embedding not in ['EMD', 'Hist']:
    ...
    if args.embedding not in ['EMD', 'Hist']:
        with open(f'./{args.embedding}_IN1K_train.ds', 'wb') as fp:
            write_embeds_to_file(fp, train_loader, dist)

        with open(f'./{args.embedding}_IN1K_val.ds', 'wb') as fp:
            write_embeds_to_file(fp, test_loader, dist)
    else:
        with open(f'./{args.embedding}_IN1K_train.ds', 'wb') as fp:
            write_signatures_to_file(fp, train_loader)

        with open(f'./{args.embedding}_IN1K_val.ds', 'wb') as fp:
            write_signatures_to_file(fp, test_loader)
